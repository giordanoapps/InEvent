<?php
	include_once("includes/check/login.php");
?>
<?php include_once("includes/html/header.php") ?>
<body>
	<?php include_once("includes/html/bar.php") ?>	
	<div id="content">
		
		<div id="termsContent" class="pageContent">
			
			<div class="titleContent">Termos de Uso</div>
			
			<div class="boardContent">
				<div class="boardContentInnerWrapper">
					
					<div class="pageContentBox shrinkInnerWrapper">
						<p>Última atualização: 10 de agosto de 2013</p>
						<p>Obrigado por ter escolhido <b>InEvent</b>. Ao acessar ou usar qualquer serviço do InEvent, você concorda com os seguintes Termos de Serviço. Se você não concorda com os seguintes Termos de Serviço, por favor não use este serviço.
						
						<p>Estúdio Trilha reserva o direito de trocar ou modificar esses Termos a qualquer momento sem aviso prévio e indicará no topo da página a data em que o documento foi modificado pela última vez. Toda e qualquer mudança efetuada nesse documento passará a ser efetiva a partir do momento em que a versão revisada for publicada em nosso site. Seu uso do serviço após a publicação da versão revisada constitui sua aceitação dos Termos. Qualquer mudança efetuada nesses Termos serão notificadas por email para os que optarem pelo recebimento deste.</p>
						
						<h3>Definições</h3>
						
						<p>InEvent: Plataforma composta por uma aplicação web localizada em "http://www.inevent.us", um aplicativo oficial para a plataforma iOS nomeado "InEvent" e um aplicativo oficial para a plataforma Android nomeado "InEvent", assim como todos seus serviços associados não referidos aqui.</p>
						
						<p>Evento: Local em que a plataforma está cadastrada e em operação, seja este um local público ou privado, de variadas atividades comerciais assim como referido na seção de cadastro nos campos "nome do evento" e "endereço".</p>
						
						<p>Contratado: Pessoa Jurídica aqui referida por Estúdio Trilha, empresa cadastrada sobre o MEI 16.916.147/0001-66, localizada na cidade de São Carlos, no Estado de São Paulo, território brasileiro. Todos os direitos da plataforma InEvent e seus serviços associados são direito exclusivo de acesso ao Estúdio Trilha.</p>
						
						<p>Contratante: Pessoa Jurídica que, dentro dos termos da lei, preencheu o formulário de cadastro para obter o InEvent em seu estabelecimento comercial.</p>
						
						<h3>Pessoa Física</h3>
						
						<p>Licença: O InEvent concede a você uma licença limitada, não exclusiva, intransferível para acessar e usar o InEvent apenas em locais legalmente autorizados, salvo conforme expressamente estabelecido nesses Termos. O uso do InEvent para fins não especificamente previstos nesse instrumento é expressamente proibido.</p>
						
						<p>Utilização: Para utilizar o aplicativo oficial do InEvent, é necessário ter um dispositivo compatível e obter o software desejado diretamente da loja oficial assim como determinada pelo dispositivo selecionado. Você concorda em jamais praticar, em qualquer instância, o ato de obter ou tentar obter os dados resguardados sigilosamente de outro estabelecimento sobre o qual não tem permissão.</p>
						
						<p>Privacidade: Todas as informações associadas as ações realizadas dentro do estabelecimento são confidenciais, ficando ao Estúdio Trilha responsável em não compartilhá-las sob nenhuma circunstância.</p>
						
						<h3>Pessoa Jurídica</h3>
						
						<p>Licença: O InEvent concede a você uma licença limitada, não exclusiva, intransferível para acessar e usar o InEvent apenas no local estabelecido do evento cujas informações foram fornecidas no momento de cadastro. Fica a cargo do contratante a responsabilidade de efetuar os pagamentos para obtenção do serviço em plena forma e funcionamento.</p>
						
						<p>Utilização: Para utilizar a plataforma InEvent, é necessário que o contratante efetue um cadastro contendo informações referentes a seu evento. Neste cadastro, o contratante certifica-se que fornecerá todos os dados corretos e verdadeiros, sob pena do crime de falsidade ideológica. Após o cadastro ser efetuado, é necessário ter um dispositivo compatível (dotado de navegador <a href="http://www.google.com/intl/pt-BR/chrome/browser/">Google Chrome</a>) e utilizar diretamente a aplicação web localizada única e exclusivamente em "http://www.inevent.us". O Estúdio Trilha reserva-se no direito de revogar acesso a plataforma caso o número de acessos e dados seja superior ao volume considerado adequado da empresa, embora ressaltamos que tal fato nunca ocorreu.</p>
						
						<p>Plano: Para acesso a plataforma InEvent durante a realização de seu evento, o contratante deverá efetuar o pagamento equivalente aos serviços contratados, podendo este ser substituído por contrato oferecido pela equipe do InEvent.</p>
						
						<p>Valores: Os valores das contas definidas acima podem variar ao longo do tempo, ficando o contratante com direito ao valor apenas no prazo máximo ao equivalente a duração de seu plano, podendo, após esse período, ter seu valor modificado, caso nenhum outro acordo esteja em vigor.</p>
						
						<p>Pagamento: Todos os planos referidos na cláusula acima devem ter seu pagamento efetuado em um prazo de no máximo 7 (SETE) dias úteis após a data de emissão da cobrança, sob pena de suspensão caso tal pagamento não seja efetuado, caso nenhum outro acordo esteja em vigor.</p>
						
						<p>Privacidade: As informações fornecidas pelo contratante ao Estúdio Trilha são de responsabilidade confidencial, ficando a cargo do Estúdio Trilha resguardar elas em local seguro.</p>

						<h3>Avisos legais</h3>

						<p>O INEVENT NÃO GARANTE QUE O INEVENT E SEUS SERVIÇOS ESTÃO LIVRES DE ERROS OU FUNCIONARÃO DE FORMA ININTERRUPTA. O INEVENT É FORNECIDO "NO ESTADO EM QUE SE ENCONTRA" E "CONFORME DISPONÍVEL". TODAS AS INFORMAÇÕES CONTIDAS NO INEVENT ESTÃO SUJEITAS A ALTERAÇÃO SEM AVISO PRÉVIO. O INEVENT NÃO PODE GARANTIR QUE O INEVENT, SEU WEBSITE, OU APLICATIVOS ESTÃO LIVRES DE VÍRUS OU DE FUNCIONALIDADES PREJUDICIAIS. O INEVENT RENUNCIA A TODAS AS GARANTIAS, EXPRESSAS OU IMPLÍCITAS, INCLUINDO QUAISQUER GARANTIAS DE PRECISÃO, NÃO VIOLAÇÃO, COMERCIABILIDADE E ADEQUAÇÃO A UMA FINALIDADE ESPECÍFICA. O INEVENT RENUNCIA A TODA E QUALQUER RESPONSABILIDADE PELOS ATOS, OMISSÕES E CONDUTAS DE TERCEIROS RELACIONADOS A SUA UTILIZAÇÃO DO INEVENT. VOCÊ É INTEGRAMENTE RESPONSÁVEL PELA SUA UTILIZAÇÃO DO INEVENT E QUAISQUER SERVIÇOS OU CONTEÚDOS A ELE RELACIONADOS. O SEU ÚNICO E EXCLUSIVO RECURSO CONTRA O INEVENT CASO VOCÊ ESTEJA INSATISFEITO COM O MESMO É INTERROMPER SEU USO. ESTA LIMITAÇÃO FAZ PARTE DO ACORDO ENTRE AS PARTES.</p>

						<p>O aviso legal acima aplica-se a quaisquer danos, responsabilidades ou prejuízos causados por qualquer falha no desempenho, erro, omissão, interrupção, eliminação, defeito, atraso na operação ou transmissão, vírus, avaria na linha de comunicação, roubo ou destruição de ou acesso não autorizado a, alteração de, ou utilização, quer por quebra de contrato, ato ilícito, negligência ou qualquer outra causa pedir.</p>

						<h3>Responsabilidade</h3>

						<p>VOCÊ CONCORDA, EM HIPÓTESE ALGUMA, A IMPUTAR AO INEVENT RESPONSABILIDADE PERANTE VOCÊ POR QUALQUER DANO, PERDA OU CAUSAS DE PEDIR, SEJA EM CONTRATO, ATO ILÍCITO (INCLUINDO, SEM LIMITAÇÃO, NEGLIGÊNCIA), OU OUTRO EM VALOR SUPERIOR AO VALOR PAGO POR VOCÊ AO INEVENT PELOS ÚLTIMOS 12 (DOZE) MESES DE SERVIÇOS OU R$99,00 (NOVENTE E NOVE REAIS), QUAL FOR MAIOR.</p>

						<h3>Indenização</h3>

						<p>Você concorda em indenizar e manter o INEVENT indene contra quaisquer reclamações, ações ou despesas (incluindo honorários advocatícios) imputadas por terceiros ou relacionadas ao seu uso do INEVENT.</p>

						<h3>Revogação</h3>

						<p>Você entende e concorda que o INEVENT poderá, a seu exclusivo critério e a qualquer momento, revogar sua conta ou seu uso de quaisquer serviço, por qualquer motivo. O INEVENT poderá, ainda, descontinuar quaisquer serviços ou limitar ou restringir qualquer acesso, por qualquer motivo. Você concorda que o INEVENT poderá tomar uma ou mais destas medida sem qualquer aviso prévio a você. No entanto, em determinados casos, caso um serviço seja descontinuado, desde que isso seja razoavelmente possível, informaremos você com antecedência para que você possa excluir seus dados pessoais relacionados ao serviço que estiver sendo descontinuado. Você entende e concorda que o INEVENT não terá qualquer responsabilidade perante você ou qualquer terceiro por qualquer rescisão do seu acesso e/ou remoção de informações relacionadas à sua conta ou serviços.</p>

						<h3>Disposições Gerais</h3>

						<p>Esses Termos serão interpretados e executados de acordo com as leis do Brasil. O tribunal do Estado de São Paulo será competente para dirimir quaisquer questões decorrentes do presente Termo de Uso, os serviços ou o conteúdo.</p>

						<p>Se qualquer disposição desse Termo de Uso for considerada ilegal, nula ou por qualquer razão inexequível, tal disposição será considerada excluída deste documento e não afetará a validade e exeqüibilidade das disposições remanescentes.</p>

						<p>Você, por este instrumento, autoriza expressamente o INEVENT a revelar e transferir suas informações armazenadas nos servidores do INEVENT ou de terceiros autorizados a qualquer terceiro em razão de qualquer restruturação societária, incluindo, sem limitação, fusão, aquisição ou venda de todos ou substancialmente todos os ativos do INEVENT. Você deverá concordar e aceitar todos os termos e condições impostos pelo novo proprietário. O INEVENT não terá qualquer responsabilidade perante você após a efetiva cessão dos ativos do INEVENT ou do Website para um terceiro interessado.</p>
					</div>
				</div>
			</div>
		</div>
    </div>
    
    <?php include_once("includes/html/wrapper.php") ?>
    
</body>
</html>